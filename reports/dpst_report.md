...
No full benchmark or external baseline claim is made. Runtime logging includes wall-clock, prediction and update time; CPU peak RSS is stored in each `timing.json`. CUDA memory is recorded when a CUDA run is used. Plots are generated by `scripts/generate_report_plots.py` from saved JSONL artifacts.
