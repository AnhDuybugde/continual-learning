...
from continual_forecasting.methods import DPST, DPSTConfig, ER
...
class PipelineTests(unittest.TestCase):
    ...
    def test_dpst_zero_controller_matches_er_update_shape(self) -> None:
        kwargs = dict(input_size=2, target_size=1, horizon=1, lr=1e-3, seed=9, device="cpu", channels=4, buffer_size=4, replay_batch_size=2)
        er = ER(**kwargs)
        dpst = DPST(**kwargs, dpst=DPSTConfig(eta_init=1e-3, beta_eta=0.0, beta_lambda=0.0))
        inputs = np.zeros((4, 2), dtype=np.float32)
        target = np.ones((1, 1), dtype=np.float32)
        er_result = er.observe_resolved(inputs, target, 0)
        dpst_result = dpst.observe_resolved(inputs, target, 0)
        self.assertTrue(torch.isfinite(torch.tensor(er_result["loss_new"])))
        self.assertAlmostEqual(dpst_result["eta"], er_result["eta"], places=8)
        self.assertEqual(dpst_result["buffer_size"], er_result["buffer_size"])
